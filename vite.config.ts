...
// Local-dev twin of the Vercel /api/explain function. Serves the same handler
// so AI enrichment works under `vite dev` without deploying. Reads GROQ_API_KEY
// from .env (server-side only — never exposed to the browser bundle).
function metabomapApiPlugin(): Plugin {
  return {
    name: 'metabomap-api',
    configureServer(server) {
      // make GROQ_API_KEY available to the handler in dev
      try {
      ...
      server.middlewares.use('/api/explain', (req, res) => {
        ...
        req.on('end', async () => {
          ...
          try {
            const { runExplain } = await import('./api/_core.mjs')
            const result = await runExplain(JSON.parse(body || '{}'))
            res.statusCode = 200
